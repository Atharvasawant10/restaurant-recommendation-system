...
def suggest():
    ...
    # partial match
    partial = zomato_df[zomato_df['name_lower'].str.contains(query, na=False)]
    if partial.empty:
        return {'suggestions': []}
        
    # get top 10 unique names
    unique_names = partial['name'].drop_duplicates().head(10).tolist()
    return {'suggestions': unique_names}
